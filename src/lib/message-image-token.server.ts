...
// downloads of images from the message-images bucket through the app origin
// (avoiding direct calls to the Supabase domain, which some networks block).
// Mirrors knowledge-file-token.server.ts.
...
export function signMessageImageToken(path: string, ttlSeconds = 3600): string {
...
export function verifyMessageImageToken(token: string): { path: string } | null {
