...
export const upsertContato = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: r, error } = data.id
      ? await context.supabase.from("contatos_enderecos").update(data).eq("id", data.id).select().single()
      : await context.supabase.from("contatos_enderecos").insert(data).select().single();
    if (error) throw new Error(error.message);
    return r;
  });
...
export const deleteContato = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await admin(context);
    const { error } = await context.supabase.from("contatos_enderecos").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
